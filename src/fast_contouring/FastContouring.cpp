...
#include <fast_contouring/cubes.h>
#include <fast_contouring/ContourGeometry.h>
...
namespace FastContouring
{
  ...
  void ContourExtractor::setVolume(const Volume& vol)
  {
    m_Data = vol;
  ...
  void ContourExtractor::classifyVertices(unsigned int k, unsigned int* cacheMemory, float isovalue) const
  {
    ...
    for (i=0; i<m_Data.xdim*m_Data.ydim; i++) {
      cacheMemory[destOffset++] = ((float)m_Data.data[sourceOffset++] < isovalue);
    }
  }
				   ...
				   float& nx, float& ny, float& nz) const
  {
    ...
    dx = (double)(m_Data.data[inputindex+negXOffset] - m_Data.data[inputindex+posXOffset]) * scaleX;
    dy = (double)(m_Data.data[inputindex+negYOffset] - m_Data.data[inputindex+posYOffset]) * scaleY;
    dz = (double)(m_Data.data[inputindex+negZOffset] - m_Data.data[inputindex+posZOffset]) * scaleZ;
					   ...
					   double R, double G, double B)
  {
	...
	for (i=0; i<numCellsX; i++) { // for every cell
	  ...
	  if (cubeCase!=0 && cubeCase!=255) {
		...
		den1 = m_Data.data[voxelOffset+offsetTable[edges[edgeID][0]]];
		den2 = m_Data.data[voxelOffset+offsetTable[edges[edgeID][1]]];
